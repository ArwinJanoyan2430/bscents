...
export default function AdminPage() {
  ...
  const addProduct = async (event) => {
    ...
    try {
      ...
      stock: Number(data.get("stock")),
        ...
        image_url: String(data.get("imageUrl") || "").trim(),
      });
  ...
  const addReview = async (event) => {
    ...
    const data = new FormData(event.currentTarget);
    try {
      await createReview({ productId: String(data.get("product")), author: String(data.get("author")).trim(), rating: Number(data.get("rating")), body: String(data.get("review")).trim() });
