...
export async function createReview({ productId, author, rating, body, imageFile }) {
  ...
  if (imageFile) {
    ...
    const { error: uploadError } = await client.storage.from("review-images").upload(path, imageFile);
    if (uploadError) throw uploadError;
    imageUrl = client.storage.from("review-images").getPublicUrl(path).data.publicUrl;
  }
...
export async function getAdminData() {
  ...
  const [productsResult, reviewsResult, ordersResult, profilesResult] = await Promise.all([
    ...
    client.from("reviews").select("id, author, rating, body, products(name)").order("created_at", { ascending: false }),
  ...
  return {
    ...
    reviews: reviewsResult.data.map((review) => ({ id: review.id, author: review.author, rating: review.rating, text: review.body, product: review.products?.name || "Deleted product" })),
...
export async function addProduct(product) {
  const { data, error } = await requireClient().from("products").insert(product).select().single();
  if (error) throw error;
